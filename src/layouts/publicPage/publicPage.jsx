    ...
    const fetchTask = async () => {
      try {
        ...
        for(let i = 0; i < task.checkList.length; i++){
          if(task.checkList[i].checked === true){
            counter++;
          }
        }
        ...
        <div className="publicPageCenterDiv">
          ...
          {task.checkList.map((item) => (
              <div className="checkList-item">
                <input type="checkbox" checked={item.checked} className="publicCheckBox"readOnly />
                {item.checked && (<img src="/assets/Tick.svg" alt="tick" className="tickImg" />)}
